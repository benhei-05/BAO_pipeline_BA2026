...
def main():
    ...
    '''
    The following file reading only exists in this code to determine the angular area and the effective redshift. 
    If these quantities are known it could be removed to make the code independent from input catalogs.
    '''
    ...
    '''
    In this code a file is called containing a pypower power spectrum object. The filename is designed to be 
    compatible with full_BAO_pipeline.py.
    '''
    
    tag = f"LRG_{os.path.basename(args.indir)}_{args.cap}_sub{subsample:.2f}"

    file_path = os.path.join(args.outdir, f"pk_{mode}_b1{b1_rec}{b1_cov}_f{f}_ssm{sigma_sm}_{tag}.npy")

    result = CatalogFFTPower.load(file_path)
    ...
    len_array = len(P0)
    print('Size of cleaned arrays:', len_array)
    ...
    np.savetxt(os.path.join(args.outdir, f"pk_rec_{tag}_b1{b1_rec:.2f}_f{f:.2f}.txt"),
    ...
    np.savetxt(os.path.join(args.outdir, f"split_pk_rec_{tag}_b1{b1_rec:.2f}_f{f:.2f}.txt"),
