...
def main():
    ...
    # --- data ---------------------------------------------------------------
    dfile = os.path.join(args.indir, f"LRG_complete_{args.cap}_clustering.dat.fits")
    ...
    rfiles = sorted(glob.glob(os.path.join(
        args.indir, f"LRG_complete_{args.cap}_*_clustering.ran.fits")))
    if not rfiles:
    ...
    result = CatalogFFTPower(
        data_positions1=data_pos, data_weights1=w,
        randoms_positions1=rand_pos, randoms_weights1=rw,
        position_type="rdd", edges=kedges, ells=ells,
        los="firstpoint", # defines line of sight; firstpoint -> uses first point
        nmesh=args.nmesh, # mesh size
        resampler="tsc", # assigns particles to mesh; 
        interlacing=2, # reduces aliasing when resampling; order 2
        boxpad=1.5, # boxsize = 1.5 times smallest box enclosing positions
        dtype="f8")
    ...
    cleaning = ~np.isnan(k)
    k, P0, P2, P4 = k[cleaning], P0[cleaning], P2[cleaning], P4[cleaning] # removing nan from arrays
    ...
    cov = covariance(b1=2.0)   # evaluate covariance matrix at this parameter
    ...
    observable.plot(fn=os.path.join(args.outdir, "plot_observable.png"))
    observable.plot_bao(fn=os.path.join(args.outdir, "plot_BAO.png"))   
                                                            
    
    tag = f"LRG_{os.path.basename(args.indir)}_{args.cap}_sub{subsample:.2f}"
    result.save(os.path.join(args.outdir, f"pk_{tag}.npy"))
    ...
    '''
    The following code block extracts the wiggle/no-wiggle split of the best fit power spectrum and saves them into
    an additional file.
    '''
    best_fit_params = {p.name: profiles.bestfit[p.name][0] for p in likelihood.varied_params}

    
    observable(**best_fit_params)

         
    theory_nobao = observable.theory_nobao # best fit no wiggle power spectrum
    theory_full = observable.theory # best fit full power spectrum
    k_obs = observable.k[0]

    
    # best fit no-wiggle multipoles
    P0_nw = theory_nobao[0]
    P2_nw = theory_nobao[1]
    P4_nw = theory_nobao[2]

    # best fit no-wiggle multipoles substracted from data multipoles
    P0_w = P0 - P0_nw
    P2_w = P2 - P2_nw
    P4_w = P4 - P4_nw

    # full theory multipoles
    P0_theo = theory_full[0]
    P2_theo = theory_full[1]
    P4_theo = theory_full[2]
    
    # best fit wiggle multipoles
    P0_w_theo = P0_theo - P0_nw
    P2_w_theo = P2_theo - P2_nw
    P4_w_theo = P4_theo - P4_nw

    

    np.savetxt(os.path.join(args.outdir, f"split_pk_prerec_{tag}.txt"),
               np.column_stack([k_obs, P0_nw, P2_nw, P4_nw, P0_w, P2_w, P4_w, P0_w_theo, P2_w_theo, P4_w_theo]),
               header="k[h/Mpc]  P0_nw  P2_nw  P4_nw  P0_w  P2_w  P4_w  P0_w_theo  P2_w_theo  P4_w_theo     [(Mpc/h)^3]")
    ...
    plt.savefig(os.path.join(args.outdir, f"pk_{tag}.png"), dpi=150)
