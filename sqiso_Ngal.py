...
S_NL_prerec = 1.7 * S_NL # rough correction of S_NL to match the prerec results
...
def P_w(k, Sigma_Silk, Sigma_NL): # model of the wiggle part of the power spectrum
    exp = np.exp(- 0.5 * k**2 * (Sigma_Silk**2 + Sigma_NL**2))
    sin = np.sin(k * l_BAO / alpha)
    return 0.05 * P_nw(k) * sin * exp


# comparing the P_w model for both pre and post reconstruction with the template P_w

model = P_w(k_grid, S_Silk, S_NL)
model_prerec = P_w(k_grid, S_Silk, S_NL_prerec)


fig_pw, ax_pw = plt.subplots()

ax_pw.plot(k_grid, P_wiggle, label='true', alpha=0.5)
ax_pw.plot(k_grid, model, label='model rec', alpha=0.5, color='red')
ax_pw.plot(k_grid, model_prerec, label='model prerec', alpha=0.5, color='purple')
ax_pw.legend()
...
def dlnP_dalpha(Sigma_Silk, Sigma_NL): # derivative following the P_w model from above
    cos = np.cos(k_grid * l_BAO / alpha)
    exp = np.exp(- 0.5 * k_grid**2 * (Sigma_Silk**2 + Sigma_NL**2))
    return - 0.05 * k_grid * l_BAO / alpha**2 * cos * exp

def F(N_gal, Sigma_Silk, Sigma_NL): # Fisher matrix approximation
    integrand = dlnP_dalpha(Sigma_Silk, Sigma_NL)**2 * w(N_gal) 
    lnk_grid = np.log(k_grid)
    return 2*np.pi * simpson(y=integrand, x=lnk_grid)


def s_qiso(N_gal, Sigma_Silk, Sigma_NL): # sigma_alpha_iso prediction from the Fisher matrix
    return 1 / np.sqrt(F(N_gal, Sigma_Silk, Sigma_NL))
...
#plotting the resulting predictions for both post and pre reconstruction
...
for N_gal in N_gal_val:
    s_qiso_val.append(s_qiso(N_gal, S_Silk, S_NL))

ax.plot(N_gal_val, s_qiso_val, label='model rec', alpha=0.5, color='red')
...
for N_gal in N_gal_val:
    s_qiso_val.append(s_qiso(N_gal, S_Silk, S_NL_prerec))

ax.plot(N_gal_val, s_qiso_val, label='model prerec', alpha=0.5, color='purple')
